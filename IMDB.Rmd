...
The linear model shown below depicts that the number of voted users,the number of critic reviews and the duration impacts IMDB score the most. The R-squared of ~ 0.29 is extremely low which suggests that the relationship between these variables is not linear.
...
The mean squared error of the model below is `mean((predict.IMDB.rf - IMDB_test$imdb_score)^2)` which is lower than the previous model `mean((predicted.rf - IMDB_test$imdb_score)^2)`. As this model uses only some of the important variables which could result into a lower root mean squared error. 
