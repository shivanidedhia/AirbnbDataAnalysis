...
## Root Mean Squared Error for the Linear Model
...
The mean squared error of the model below is `r mean((predict.IMDB.rf - IMDB_test$imdb_score)^2)` which is lower than the previous model `r mean((predicted.rf - IMDB_test$imdb_score)^2)`. As this model uses only some of the important variables which could result into a lower root mean squared error. 
...
## Root Mean Squared for the Random Forest

The root mean sqaured for this model is ~ .67 which is considered to be a good model. The difference between the predicted and the actual values is low.
...
So, based on the visualizations we can conclude that content rating does not have much impact on the IMDB score while the IMDB score by countries shows number of movies which has lower IMDB score are the most in the chart. As expected, IMDB score cannot define the profitability of the movies. 
