...
that impact the success and acceptance of the movie along with the profitability. 
...
Only a handful of directors have over 15 movies directed in this data set. Steven Spielberg is the only director to have ~ 24 movies directed. The chart below shows average imdb score for directors with 15 or more directed movies. The imdb score is above 5.5 for directors with more than 15 movies. Most directors have received a higher imdb score that shows the number of movies directed has a slight impact on the imdb score.
...
Technically, the movies with higher imdb score should generate higher net profit. But this is not true always. There are many movies who has very good imdb score but they did not generate much profit. So, imdb score cannot be a sole factor to consider the net profit. Sometimes, this score can be litte confusing as well.
...
In order to get the most important variables, we are diving the dataset into two. The 80% of the data are considered as training dataset while the rest 20% are used for testing purpose. 
...
We will be suing the random forest to identify the most important variables. For the sake of our project, we are building a random forest with 100 trees. We are using all variables for now so that we wont miss anything. The model will be conducted on the training data set. 
...
So after running the above codes and implementing these on a chart, we can see that the most important variable is the number of voter user. The reason is quite obvious because the rating only generates when people vote or give reviews for the movies. There might be a good number of people who avoid giving reviews. But there are also a good number of people who vote for the best movies which is one of the perfect factors while calculating the imdb score. Second most important factor is the duration of the movies. This is quite interesting because this is not something which is easily understandable, however the logical rationale could be the long hours movies are generally high budgeted ones as well with more reknowned casts. So, probably the quality of the movies with longer duration are better than the other ones. The next factor is the facebook like. This is actually a difficult predictor to assume however, we can say that people give like in facebook when the enjoy something. So, they might have a tendency to provide good imdb rating as well.

Next three variables are close to each other and these are budget, genres and number of user reviews. A high budgeted movie will typically have a tendency to get high imdb scores because they are usually created with lots of resourses and best technical support with a very good story. Genres also have an impact because some genres are more attractive to users than others. Typically, action and thriller movies are more preferred to many viewers. For obvious reasons, number of user reviews are important. These users directly rate the movies in the imdb site. So, certainly this will be one of the most important factors.
